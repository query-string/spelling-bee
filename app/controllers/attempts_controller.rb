class AttemptsController < ApplicationController
  attr_reader :correct_word, :input_word
  ...
  def new
    ...
    if words.empty?
      words = @list.words.random(current_person)
      current_person.update(proficiency_level: :random)
    end
  ...
  def result
    if input_word == correct_word.name.downcase
